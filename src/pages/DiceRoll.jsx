...
import DiceRollBoard from "../components/boards/DiceRollBoard"
...
const DiceRoll = () => {
  ...
  useEffect(() => {
    const URL = `${process.env.REACT_APP_BACKEND_URL}/d-r`
    const socket = io(URL)
    setSocket(socket)
  ...
  if (state) return <DiceRollBoard state={state} socket={socket} account={account} />
